...
const whisperCppOutput = await transcribe({
  ...
  inputPath: path.join(process.cwd(), "public", "bisontes_16k.wav"),
...
fs.writeFileSync(
  path.join(process.cwd(), "public", "captions_bisontes.json"),
  JSON.stringify(captions, null, 2),
);

const plain = captions.map((c) => c.text).join("");
fs.writeFileSync(path.join(process.cwd(), "transcript_bisontes.txt"), plain.trim());

console.log("\n=== TRANSCRIPT ===\n");
console.log(plain.trim());
console.log("\n=== DONE === captions_bisontes.json written:", captions.length, "tokens");
