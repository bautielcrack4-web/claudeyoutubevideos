...
const whisperCppOutput = await transcribe({
  ...
  inputPath: path.join(process.cwd(), "public", "animales_16k.wav"),
...
fs.writeFileSync(
  path.join(process.cwd(), "public", "captions_animales.json"),
  JSON.stringify(captions, null, 2),
);

const plain = captions.map((c) => c.text).join("");
fs.writeFileSync(path.join(process.cwd(), "transcript_animales.txt"), plain.trim());

console.log("\n=== TRANSCRIPT ===\n");
console.log(plain.trim());
console.log("\n=== DONE === captions_animales.json written:", captions.length, "tokens");
