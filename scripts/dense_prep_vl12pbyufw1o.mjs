// dense_prep_vl12pbyufw1o.mjs — toma public/broll/dense_vl12pbyufw1o.json ({at,query}) →
// resuelve cada 'at' al ms del caption, afina a ~2.3s, asigna dNNN y escribe:
//   public/broll/dense_thinned_vl12pbyufw1o.json
//   public/broll/shots_dense_vl12pbyufw1o.json  (para fetchstock)
//   src/_fed6/VideoEdit/federer_vl12pbyufw1o_broll.ts  (FEDZ_BROLL, track contiguo)
import fs from "fs";
...
const CW = CAPARR.map((x) => ({ t: norm(x.text), s: (x.startMs || 0) / 1000 }));
...
for (const b of dense) {
  let t = b.t != null ? b.t : findMs(b.at, Math.max(0, cursor - 1));
  if (t == null) { miss++; continue; }
  if (t < cursor + MINGAP) continue;
  cursor = t;
  kept.push({ name: `${SLUG}_d${String(kept.length).padStart(3, "0")}`, query: b.query, t: +t.toFixed(2) });
}
