// dense_prep_vr4b7bsn4mcv.mjs — toma public/broll/dense_vr4b7bsn4mcv.json ({at,query}) →
// resuelve cada 'at' al ms del caption, afina a ~2.3s, asigna dNNN y escribe:
//   public/broll/dense_thinned_vr4b7bsn4mcv.json
//   public/broll/shots_dense_vr4b7bsn4mcv.json  (para fetchstock)
//   src/_fed6/VideoEdit/federer_vr4b7bsn4mcv_broll.ts  (FEDZ_BROLL, track contiguo)
import fs from "fs";
const SLUG = "vr4b7bsn4mcv";
const MINGAP = 2.2;
const _rawcaps = JSON.parse(fs.readFileSync(`public/captions_${SLUG}.json`, "utf8"));
const caps = _rawcaps.words || _rawcaps;
...
const VEND = (CW[CW.length - 1]?.s || 1730) + 2;
