// dense_prep4.mjs — igual que dense_prep3.mjs pero para el slug federer4 (PIERNAS).
// Toma public/broll/dense_federer4.json ({at,query}) → ancla cada 'at' al ms del
// caption, afina a ~2.3s, asigna dNNN y escribe:
//   public/broll/dense_thinned4.json  ·  public/broll/stock_dense4.json (fetchstock)
//   src/VideoEdit/federer4_broll.ts   (FED4_BROLL, capa continua para Main_federer4)
import fs from "fs";
const MINGAP = 2.2;
...
const dense = JSON.parse(fs.readFileSync("public/broll/dense_federer4.json", "utf8"));
...
fs.writeFileSync("public/broll/dense_thinned4.json", JSON.stringify(kept, null, 1));
fs.writeFileSync("public/broll/stock_dense4.json", JSON.stringify(stock, null, 1));
...
fs.writeFileSync("src/VideoEdit/federer4_broll.ts",
  `// AUTO-GENERADO por scripts/dense_prep4.mjs — b-roll denso (footage real cada ~2.5s).\n` +
  `export const FED4_BROLL: { name: string; src: string; start: number; dur: number; query: string }[] = ${JSON.stringify(broll)};\n`);
