// pace_aceite.mjs — pacing (tomas ≤5s ancladas a palabra) + emisión de TODOS los prompts.
// Igual que pace_restaura pero: (a) slug aceite, (b) aceite NO tiene imágenes base, así
// que emite prompts_aceite_all.json con TODAS las tomas (base 207 + toma-2/3 nuevas).
//   node pace_aceite.mjs
//   node gen_gptimage.mjs prompts_aceite_all.json public/img 1792x1008 low
//   node build_aceite.mjs && node beatsheet.mjs beatsheet/aceite.json
import fs from "fs";

const TARGET = 2.6, MIN = 1.7;
const caps = JSON.parse(fs.readFileSync("public/captions_aceite.json", "utf8"));
const beats = JSON.parse(fs.readFileSync("_v3/aceite_beats.json", "utf8"));
...
fs.copyFileSync("_v3/aceite_beats.json", "_v3/aceite_beats_pre_pace.json");
fs.writeFileSync("_v3/aceite_beats.json", JSON.stringify(newBeats, null, 1));
// prompts de TODAS las tomas (base + nuevas)
const prompts = newBeats.map((b) => ({ name: b.name, prompt: WRAP(b.desc) }));
fs.writeFileSync("prompts_aceite_all.json", JSON.stringify(prompts, null, 1));
