// gen_romnoc.mjs — convierte public/plan_romnoc.json (6 agentes Haiku + densificado)
...
// Look THEME_MEDICO. Slug = romnoc.
//   Salidas: src/VideoEdit/romnoc_broll.ts  (b-roll CONTIGUO · RawShot)
//            src/VideoEdit/romnoc_beats.ts  (componentes · renderRecalComp)
//            src/VideoEdit/romnoc_hooks.ts   (scrims + emphasis + endcard)
//            public/broll/stock_romnoc.json  (mapa name→query para fetch_pexels)
import fs from "fs";
const plan = JSON.parse(fs.readFileSync("public/plan_romnoc.json", "utf8"));
...
const AVSET = new Set();
for (let i = 1; i <= 4; i++) {
  const f = `public/img/prompts_romnoc_${i}.json`;
...
for (const b of plan) {
  ...
  if (b.kind === "broll") {
    const i = bn++;
    const name = `rnrb${String(i).padStart(3, "0")}`;
    const abr = `rnabr${String(i).padStart(3, "0")}`;
...
const wr = (f, name, val, type = "any[]") =>
  fs.writeFileSync(`src/VideoEdit/${f}`, `// AUTO-GENERADO por gen_romnoc.mjs\nexport const ${name}: ${type} = ${JSON.stringify(val)};\n`);
wr("romnoc_broll.ts", "ROMNOC_BROLL", broll, "{name:string;src:string;start:number;dur:number;img?:boolean;av?:boolean}[]");
wr("romnoc_beats.ts", "ROMNOC_COMPS", comps);
fs.writeFileSync("src/VideoEdit/romnoc_hooks.ts",
  `// AUTO-GENERADO por gen_romnoc.mjs — overlays del video Arándano/Ojos.\n` +
  `export const ROMNOC_SCRIMS: {start:number;dur:number;word:string;accent:string}[] = ${JSON.stringify(scrims)};\n` +
  `export const ROMNOC_EMPH: {from:number;to:number;text:string}[] = ${JSON.stringify(emphases)};\n` +
  `export const ROMNOC_ENDCARD = ${endcardT.toFixed(2)};\n` +
  `export const ROMNOC_END = ${VEND.toFixed(2)};\n`);

fs.mkdirSync("public/broll", { recursive: true });
fs.writeFileSync("public/broll/stock_romnoc.json", JSON.stringify(stock, null, 1));
