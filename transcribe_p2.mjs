...
console.log("Transcribing Part 2 (Spanish)...");
const whisperCppOutput = await transcribe({
  model: MODEL,
  whisperPath: to,
  whisperCppVersion: "1.5.5",
  inputPath: path.join(process.cwd(), "public", "p2", "audio.wav"),
...
fs.writeFileSync(
  path.join(process.cwd(), "public", "p2", "captions.json"),
  JSON.stringify(captions, null, 2),
);

// Also write a plain readable transcript for review.
const plain = captions.map((c) => c.text).join("");
fs.writeFileSync(path.join(process.cwd(), "transcript_p2.txt"), plain.trim());

console.log("\n=== TRANSCRIPT P2 ===\n");
console.log(plain.trim());
console.log("\n=== DONE === p2/captions.json written:", captions.length, "tokens");
