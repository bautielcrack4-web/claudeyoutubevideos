// pace_sellador.mjs — pacing (tomas ≤5s ancladas a palabra) + emisión de TODOS los prompts.
// Igual que pace_restaura pero: (a) slug sellador, (b) sellador NO tiene imágenes base, así
// que emite prompts_sellador_all.json con TODAS las tomas (base 207 + toma-2/3 nuevas).
//   node pace_sellador.mjs
//   node gen_gptimage.mjs prompts_sellador_all.json public/img 1792x1008 low
//   node build_sellador.mjs && node beatsheet.mjs beatsheet/sellador.json
import fs from "fs";

const TARGET = 2.6, MIN = 1.7;
const caps = JSON.parse(fs.readFileSync("public/captions_sellador.json", "utf8"));
const beats = JSON.parse(fs.readFileSync("_v3/sellador_beats.json", "utf8"));
...
fs.copyFileSync("_v3/sellador_beats.json", "_v3/sellador_beats_pre_pace.json");
fs.writeFileSync("_v3/sellador_beats.json", JSON.stringify(newBeats, null, 1));
// prompts de TODAS las tomas (base + nuevas)
const prompts = newBeats.map((b) => ({ name: b.name, prompt: WRAP(b.desc) }));
fs.writeFileSync("prompts_sellador_all.json", JSON.stringify(prompts, null, 1));
