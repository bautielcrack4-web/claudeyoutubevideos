...
import { CUES } from "./cues_azotea.gen";

// ── EL CONSTRUCTOR LIBRE · MADERA #2 ─────────────────────────────────────────
// "La Madera Que Rompe El Hacha: 100 Años Sin Pudrirse" (azotea · especie/historia).
// Avatar "Tomás" (azotea_opt.mp4). ~23 min. Avatar OCULTO en demos/ScrollDocs.
...
export const TOTAL_FRAMES_AZ = Math.round(VIDEO_END * 30);
...
const STINGER_SECS = ["falla","cancer","prep","receta","aplica","prueba","usos","faq","errores","industria","cta","cierre"];
const SECTION_KEYS = new Set(CUES.filter((c) => /_0$/.test(c.key) && STINGER_SECS.includes(sectionOf(c.key))).map((c) => c.key));

export const MainAzotea: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: COLORS.bg0 }}>
      <CinematicWrap grain={0} vignette={0}>
        <AbsoluteFill style={{ backgroundColor: COLORS.bg0 }}>
          ...
          <AvatarLayer src="azotea_opt.mp4" windows={AVATAR_WINDOWS} accent={COLORS.accent} />
          {(() => {
            ...
            return (
              <Sequence from={sec(from)} durationInFrames={sec(dur)} layout="none">
                <AvatarScrimText durationInFrames={sec(dur)} setup="El agua no toca tu azotea" impact="NO SE MOJA" impactAccent="amber" fontSize={150} />
              </Sequence>
            );
          })()}
          {[...SECTION_KEYS].map((k) => {
