...
import { TEROMERO_BROLL } from "./teromero_broll";
import { TEROMERO_COMPS } from "./teromero_beats";
import { TEROMERO_SCRIMS, TEROMERO_EMPH, TEROMERO_ENDCARD, TEROMERO_END } from "./teromero_hooks";

// ── DR. FEDERER · #2 "Teromero / firmeza de la piel" ────────────────────────────
...
export const TOTAL_FRAMES_TEROMERO = Math.round(TEROMERO_END * 30);

function buildWindows(): AvatarWindow[] {
  const CORNER: AvatarWindow["mode"] = "cornerTR";
  const HOOK = 6;
  const brollIv: [number, number][] = [];
  for (const b of TEROMERO_BROLL) { if ((b as any).av) continue; brollIv.push([b.start, b.start + b.dur]); }
  ...
  const hideIv: [number, number][] = [
    ...TEROMERO_COMPS.filter((c: any) => !OVERLAY_KINDS.has(c.kind) || (c as any).av).map((c: any) => [c.start, c.start + c.dur] as [number, number]),
    ...TEROMERO_BROLL.filter((b: any) => b.av).map((b: any) => [b.start, b.start + b.dur] as [number, number]),
    ...TEROMERO_EMPH.map((e) => [e.from, e.to] as [number, number]),
  ];
  ...
  for (let t = HOOK + 28; t < TEROMERO_END - 12; t += 50) idFull.push([t, t + 10]);
  ...
  const sorted = [...pts].filter((t) => t >= 0 && t < TEROMERO_END).sort((a, b) => a - b);
...
const KIND_ORD: number[] = (() => {
  ...
  return TEROMERO_COMPS.map((c: any) => (seen[c.kind] = (seen[c.kind] ?? -1) + 1));
})();
...
const WHOOSH_TS = TEROMERO_COMPS.filter((c: any) => SECTION_KINDS.has(c.kind)).map((c: any) => c.start);
const BOOM_TS = (TEROMERO_COMPS.find((c: any) => c.kind === "plt") || TEROMERO_COMPS.find((c: any) => c.kind === "avexplain") || { start: 6 }).start;

export const MainTeromero: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      {/* CAPA 1 — b-roll con cámara viva */}
      {TEROMERO_BROLL.map((b) => (
      ...
      {/* CAPA 2 — avatar */}
      <AvatarLayer src="teromero_opt.mp4" windows={AVATAR_WINDOWS} accent={TEAL} />

      {/* CAPA 3 — componentes */}
      {TEROMERO_COMPS.map((c: any, i: number) => (
      ...
      {/* OVERLAY — emphasis */}
      {TEROMERO_EMPH.map((e, i) => (
        <Sequence key={`e_${i}`} from={sec(e.from)} durationInFrames={sec(Math.max(1.4, e.to - e.from))} layout="none">
          <EmphasisMoment durationInFrames={sec(Math.max(1.4, e.to - e.from))} avatarFromSec={e.from} text={e.text} avatarSrc="teromero_opt.mp4" />
        </Sequence>
      ))}

      {/* ENDCARD */}
      <Sequence from={sec(TEROMERO_ENDCARD)} durationInFrames={sec(Math.max(2, TEROMERO_END - TEROMERO_ENDCARD))} layout="none">
        <Endcard durationInFrames={sec(Math.max(2, TEROMERO_END - TEROMERO_ENDCARD))} />
      </Sequence>
      ...
      <Sequence from={sec(TEROMERO_ENDCARD)} layout="none"><Audio src={staticFile("sfx/music_federer.mp3")} startFrom={sec(20)} volume={(f) => interpolate(f / 30, [0, 2, 34, 42], [0, 0.13, 0.13, 0.03], { extrapolateLeft: "clamp", extrapolateRight: "clamp" })} /></Sequence>
    </AbsoluteFill>
  );
};
