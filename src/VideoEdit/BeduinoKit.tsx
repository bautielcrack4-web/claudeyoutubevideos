...
// Kit de escenas con FOTO para el video beduino (nicho jardinería/casero:
// la imagen real ES el contenido). Las fotos viven en public/bed/NNN.jpg y se
// referencian por número. Todo full-screen y opaco (cubre el avatar; el audio
// del avatar sigue sonando debajo).

const img = (n: string) => `bed/${n}.jpg`;

// ----- Lower-third caption sobre foto a pantalla completa (foto nítida) -----
...
export const PhotoCards: React.FC<{
  durationInFrames: number;
  title?: string;
  items: { n: string; label: string; caption?: string; accent?: string }[];
  hue?: "blue" | "amber" | "red";
        ...
        <div style={{ display: "flex", gap: 40, justifyContent: "center" }}>
          {items.map((it, i) => {
            ...
            return (
              ...
              >
                <DepthCard accent={accent} image={img(it.n)} width={cardW} height={cardH} radius={30} blur={3} darken={0.42} seed={i + 1}>
                  <div
                    style={{
                      position: "absolute",
                      left: 0,
...
export const QuoteScene: React.FC<{
  ...
  attribution?: string;
  accent?: string;
}> = ({ durationInFrames, n, quote, attribution, accent = COLORS.amber }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
...
// ----- Checklist de ítems con tilde verde sobre foto (recap "cero...") -----
export const CheckList: React.FC<{
  durationInFrames: number;
  title?: string;
  items: string[];
  n?: string; // foto de fondo opcional
  accent?: string;
  hue?: "blue" | "amber" | "red";
}> = ({ durationInFrames, title, items, n, accent = COLORS.good, hue = "blue" }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const titleIn = spring({ frame, fps, config: { damping: 18, mass: 0.7 } });
  ...
  return (
    <SceneFrame
      ...
      hue={hue}
      bg={n ? "image" : "black"}
      image={n ? img(n) : undefined}
      imageBlur={n ? 8 : undefined}
      imageDarken={n ? 0.66 : undefined}
    >
      <div style={{ ...glass("dark"), padding: "44px 56px", borderRadius: 28, minWidth: 760, maxWidth: 1100 }}>
        {title && (
          <div
            style={{
              fontFamily: FONT_STACK,
          ...
          {items.map((it, i) => {
            ...
            return (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 22, opacity: s, transform: `translateX(${(1 - s) * -30}px)` }}>
                ...
                <div style={{ fontFamily: FONT_STACK, fontSize: 38, fontWeight: 600, color: COLORS.text }}>{it}</div>
              </div>
            );
          })}
        </div>
