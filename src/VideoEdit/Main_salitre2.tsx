...
import { CUES } from "./cues_salitre2.gen";

// ── EL CONSTRUCTOR LIBRE · MADERA #2 ─────────────────────────────────────────
// "La Madera Que Rompe El Hacha: 100 Años Sin Pudrirse" (salitre2 · especie/historia).
// Avatar "Tomás" (salitre2_opt.mp4). ~23 min. Avatar OCULTO en demos/ScrollDocs.
...
export const TOTAL_FRAMES_SA2 = Math.round(VIDEO_END * 30);
...
const STINGER_SECS = ["quees","ignorar","aspirina","antibiotico","filtracion","lluvia","capilaridad","inyeccion","plata","esconden","faq","cta","cierre"];
const SECTION_KEYS = new Set(CUES.filter((c) => /_0$/.test(c.key) && STINGER_SECS.includes(sectionOf(c.key))).map((c) => c.key));

export const MainSalitre2: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: COLORS.bg0 }}>
      <CinematicWrap grain={0} vignette={0}>
        <AbsoluteFill style={{ backgroundColor: COLORS.bg0 }}>
          ...
          <AvatarLayer src="salitre2_opt.mp4" windows={AVATAR_WINDOWS} accent={COLORS.accent} />
          {(() => {
            ...
            return (
              <Sequence from={sec(from)} durationInFrames={sec(dur)} layout="none">
                <AvatarScrimText durationInFrames={sec(dur)} setup="El salitre siempre vuelve" impact="POR ESTO" impactAccent="amber" fontSize={160} />
              </Sequence>
            );
          })()}
          {[...SECTION_KEYS].map((k) => {
