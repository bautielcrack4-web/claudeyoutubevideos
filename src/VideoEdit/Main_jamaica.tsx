...
import { JAMAICA_BROLL } from "./jamaica_broll";
import { JAMAICA_COMPS } from "./jamaica_beats";
import { JAMAICA_SCRIMS, JAMAICA_EMPH, JAMAICA_ENDCARD, JAMAICA_END } from "./jamaica_hooks";

// ── DR. FEDERER · #2 "Jamaica / firmeza de la piel" ────────────────────────────
...
export const TOTAL_FRAMES_JAMAICA = Math.round(JAMAICA_END * 30);

function buildWindows(): AvatarWindow[] {
  const CORNER: AvatarWindow["mode"] = "cornerTR";
  const HOOK = 6;
  const brollIv: [number, number][] = [];
  for (const b of JAMAICA_BROLL) { if ((b as any).av) continue; brollIv.push([b.start, b.start + b.dur]); }
  ...
  const hideIv: [number, number][] = [
    ...JAMAICA_COMPS.filter((c: any) => !OVERLAY_KINDS.has(c.kind) || (c as any).av).map((c: any) => [c.start, c.start + c.dur] as [number, number]),
    ...JAMAICA_BROLL.filter((b: any) => b.av).map((b: any) => [b.start, b.start + b.dur] as [number, number]),
    ...JAMAICA_EMPH.map((e) => [e.from, e.to] as [number, number]),
  ];
  ...
  for (let t = HOOK + 28; t < JAMAICA_END - 12; t += 50) idFull.push([t, t + 10]);
  ...
  const sorted = [...pts].filter((t) => t >= 0 && t < JAMAICA_END).sort((a, b) => a - b);
...
const KIND_ORD: number[] = (() => {
  ...
  return JAMAICA_COMPS.map((c: any) => (seen[c.kind] = (seen[c.kind] ?? -1) + 1));
})();
...
const WHOOSH_TS = JAMAICA_COMPS.filter((c: any) => SECTION_KINDS.has(c.kind)).map((c: any) => c.start);
const BOOM_TS = (JAMAICA_COMPS.find((c: any) => c.kind === "plt") || JAMAICA_COMPS.find((c: any) => c.kind === "avexplain") || { start: 6 }).start;

export const MainJamaica: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      {/* CAPA 2 — avatar */}
      <AvatarLayer src="jamaica_opt.mp4" windows={AVATAR_WINDOWS} accent={TEAL} />

      {/* CAPA 3 — componentes */}
      {JAMAICA_COMPS.map((c: any, i: number) => (
      ...
      {/* OVERLAY — emphasis */}
      {JAMAICA_EMPH.map((e, i) => (
        <Sequence key={`e_${i}`} from={sec(e.from)} durationInFrames={sec(Math.max(1.4, e.to - e.from))} layout="none">
          <EmphasisMoment durationInFrames={sec(Math.max(1.4, e.to - e.from))} avatarFromSec={e.from} text={e.text} avatarSrc="jamaica_opt.mp4" />
        </Sequence>
      ))}

      {/* ENDCARD */}
      <Sequence from={sec(JAMAICA_ENDCARD)} durationInFrames={sec(Math.max(2, JAMAICA_END - JAMAICA_ENDCARD))} layout="none">
        <Endcard durationInFrames={sec(Math.max(2, JAMAICA_END - JAMAICA_ENDCARD))} />
      </Sequence>
      ...
      <Sequence from={sec(JAMAICA_ENDCARD)} layout="none"><Audio src={staticFile("sfx/music_federer.mp3")} startFrom={sec(20)} volume={(f) => interpolate(f / 30, [0, 2, 34, 42], [0, 0.13, 0.13, 0.03], { extrapolateLeft: "clamp", extrapolateRight: "clamp" })} /></Sequence>
    </AbsoluteFill>
  );
};
