...
import { CUES } from "./cues_estafas.gen";

// ── CANAL "BEN RETIRADO" · Video 3 · "Las estafas que vacían la cuenta" ──
// Look ALARMA (theme_ben). Avatar Ben (estafas_opt.mp4 provee el audio). TEMA UNIVERSAL
// hispano. KeyPhrase palabra-por-palabra + PNGs flotando + MistakeCard×8 + ProtectionTool×6
// + journey/annotated/callout. Sync milimétrico (captions alineadas). ~24.7 min.

const VIDEO_END = Math.max(...CUES.map((c) => c.start + c.dur)) + 1.5;
export const TOTAL_FRAMES_ES = Math.round(VIDEO_END * 30);

const GRADE: GradeRange[] = [
  { from: 0, to: 158, tint: "#E11507", strength: 0.08 },        // hook: rojo alarma
  { from: 158, to: 526, tint: "#C8904A", strength: 0.05 },      // Aldo / Rosa: ámbar cálido
  { from: 526, to: 1034, tint: "#E11507", strength: 0.06 },     // las 8 estafas + patrón: rojo
  { from: 1034, to: 1224, tint: "#1FBF4F", strength: 0.05 },    // la defensa: verde a salvo
  { from: 1224, to: VIDEO_END, tint: "#FFC400", strength: 0.05 }, // cierre / firma: oro
];

// keyphrase/floatprop/diorama/struct = full-screen con su propio fondo → ocultan avatar.
// statpills/callout = overlay sobre el avatar → avatar VISIBLE.
const HIDE = new Set([
...
const FULL_SECS = new Set(["benintro", "bridge", "patron", "cta_share", "cta_sub", "regla", "yacaiste", "epi", "cta_final", "outro"]);
...
// stingers + swells en los arranques de sección importantes
const STINGER_KEYS = ["aldo_0", "estafa1_0", "patron_0", "regla_0", "yacaiste_0", "firma_0", "tease_0"];
const SECTION_KEYS = new Set(CUES.filter((c) => STINGER_KEYS.includes(c.key)).map((c) => c.key));

export const MainEstafas: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: COLORS.bg0 }}>
      <CinematicWrap handheld={0.7} grain={0.05}>
        <AbsoluteFill style={{ backgroundColor: COLORS.bg0 }}>
          ...
          <AvatarLayer src="estafas_opt.mp4" windows={AVATAR_WINDOWS} accent={COLORS.accent} />
          {(() => {
            ...
            return (
              <Sequence from={sec(from)} durationInFrames={sec(dur)} layout="none">
                <AvatarScrimText durationInFrames={sec(dur)} setup="Si tienes más de 60…" impact="Una llamada te VACÍA la cuenta" impactAccent="danger" fontSize={108} />
              </Sequence>
            );
          })()}
