...
import { CUES } from "./cues_barrera.gen";

// ── VIDEO 3 · EL CONSTRUCTOR LIBRE — "2 metales = 0 plagas" ──
// Avatar "Tomás" (barrera_opt.mp4 = audio narración) + flujo TUTORIAL denso
// (gpt-image-2 16:9 imperfecto + clips reales + diagramas). 3 injertos. ~18.8 min.

const VIDEO_END = Math.max(...CUES.map((c) => c.start + c.dur)) + 1.5;
export const TOTAL_FRAMES_BRR = Math.round(VIDEO_END * 30);

const GRADE: GradeRange[] = [
  { from: 0, to: 90, tint: "#7C8A5A", strength: 0.06 },        // hook: verde huerta
  { from: 90, to: 320, tint: "#B0833F", strength: 0.05 },      // barrera: tierra
  { from: 320, to: 600, tint: "#7C8A5A", strength: 0.05 },     // defensas: salvia
  { from: 600, to: 980, tint: "#B0833F", strength: 0.05 },     // empezar/plan: tierra
...
export const MainBarrera: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: COLORS.bg0 }}>
      <CinematicWrap handheld={0.8} grain={0.06}>
        <AbsoluteFill style={{ backgroundColor: COLORS.bg0 }}>
          ...
          <AvatarLayer src="barrera_opt.mp4" windows={AVATAR_WINDOWS} accent={COLORS.accent} />
          {(() => {
            ...
            return (
              <Sequence from={sec(from)} durationInFrames={sec(dur)} layout="none">
                <AvatarScrimText durationInFrames={sec(dur)} setup="Sin veneno. Sin matar una abeja." impact="2 metales = 0 plagas" impactAccent="good" fontSize={130} />
              </Sequence>
            );
          })()}
