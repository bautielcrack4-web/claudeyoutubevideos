...
// Cues generados por build_cmemacetas.mjs. Ken-Burns aleatorio por plano (regla dura).
...
import {CUES_CMEMACETAS, TOTAL_FRAMES_CMEMACETAS, AVATAR_FRAMES_CMEMACETAS} from "./cues_cmemacetas.gen";
...
const AvatarFloor: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{backgroundColor: "#10120d", overflow: "hidden"}}>
      <Loop durationInFrames={AVATAR_FRAMES_CMEMACETAS}>
        <OffthreadVideo src={staticFile("cmemacetas_opt.mp4")} muted style={{width: "100%", height: "100%", objectFit: "cover", transform: `scale(${scale.toFixed(4)})`}} />
      </Loop>
    </AbsoluteFill>
  );
};
...
// Clip UNA sola vez, NUNCA en bucle (regla del creador). Para llenar el slot se BAJA la velocidad,
// a lo sumo 0,5x. Si el slot no se llena ni a 0,5x, el build ya lo mandó a foto (no llega acá).
const Clip: React.FC<{src: string; frames: number; seed: number}> = ({src, frames, seed}) => {
  const {durationInFrames} = useVideoConfig();
  const rate = Math.max(0.5, Math.min(1, frames / Math.max(1, durationInFrames)));
  return (
    <AbsoluteFill style={{backgroundColor: "#10120d", overflow: "hidden"}}>
...
export {TOTAL_FRAMES_CMEMACETAS};

export const MainCmemacetas: React.FC = () => (
  <AbsoluteFill style={{backgroundColor: "#10120d"}}>
    <AvatarFloor />
    {CUES_CMEMACETAS.map((c) => (
      <Sequence key={c.key} from={c.start} durationInFrames={c.dur} premountFor={30}>
        {c.kind === "clip" ? (
          <Clip src={c.src} frames={c.frames ?? 150} seed={c.seed} />
        ) : c.kind === "cta" ? (
          <CtaStill src={c.src} />
        ) : (
    ...
    <Audio src={staticFile("cmemacetas.m4a")} />
  </AbsoluteFill>
);
