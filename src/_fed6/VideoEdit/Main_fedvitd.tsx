...
import { FEDZ_BEATS } from "./federer_fedvitd_beats";
import { FEDZ_BROLL } from "./federer_fedvitd_broll";
import { TALKSZ } from "./federer_fedvitd_hooks";
import { renderFederer2Comp, COMP2_KINDS } from "./FedererComponents2";

// ── CANAL "Federer Consejos" · 1 VITAMINA PARA LAS PIERNAS (vitamina D) ────────
// Avatar en 3 modos, CERO recuadro: FULL · HIDDEN (visual full) · SPLIT halfR.
const TEAL = "#12B3AE";
const BG = "#0E1D23";
const AVA = "fedvitd_opt.mp4";
...
export const TOTAL_FRAMES_FEDVITD = Math.round(VIDEO_END * 30);
...
// FULL breve del avatar: arranque de secciones fuertes (los talks ya van full por TALKSZ)
const FULL_AT: number[] = [];
FEDZ_BEATS.filter((b: any) => /^(gancho|error_intro|error1|error2|error3|protocolo_intro|regalo3_teaser|guia3)$/.test(b.key) && (b.id.endsWith("_0")))
...
function buildWindows(): AvatarWindow[] {
  ...
  // HOOK: avatar FULL 2.2s y después HIDDEN durante el scrim
  const HOOK_FULL = 2.2;
  const HOOK_END = 7.6;
...
export const MainFedvitd: React.FC = () => {
  const hookStart = 2.2;
  const hookDur = 5.2;
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      <Sequence from={sec(hookStart)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)} setup="Una sola vitamina mantiene tus piernas fuertes… incluso a los 95." impact="NO ES LA EDAD" accentColor="#12B3AE" font={F_INTER} fontSize={130} />
      </Sequence>
