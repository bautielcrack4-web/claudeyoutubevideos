...
import { FEDZ_BEATS } from "./federer_vhy9s3dd4wqx_beats";
import { FEDZ_BROLL } from "./federer_vhy9s3dd4wqx_broll";
import { TALKSZ } from "./federer_vhy9s3dd4wqx_hooks";
import { renderFederer2Comp, COMP2_KINDS } from "./FedererComponents2";

// ── CANAL "Federer Consejos Salud" · LA FRUTA Nº1 PARA LA CIRCULACIÓN EN LAS PIERNAS +60 ──
// Avatar en 3 modos, CERO recuadro (feedback): FULL · HIDDEN (visual full) · SPLIT halfR.
const TEAL = "#12B3AE";
const BG = "#0E1D23";
const AVA = "vhy9s3dd4wqx_opt.mp4";
...
export const TOTAL_FRAMES_VHY = Math.round(VIDEO_END * 30);
...
FEDZ_BEATS.filter((b: any) => /^(hook|promesa|presenta|nobel|paradoja|fruta_uva|ben1|ben3|mito|comer|limites|error|recap|cta_com|cierre|close)$/.test(b.key) && (b.id.endsWith("_0")))
...
function buildWindows(): AvatarWindow[] {
  ...
  for (const b of content) {
    const forceHidden = /libro|cocina|guia|pareja|abuelo|farmacia|jugo/.test(b.src || "");
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      <Sequence from={sec(hookStart)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)} setup="Una sola fruta le devolvió la circulación a sus piernas…" impact="LA FRUTA Nº1 PARA TUS PIERNAS" accentColor="#12B3AE" font={F_INTER} fontSize={116} />
      </Sequence>
