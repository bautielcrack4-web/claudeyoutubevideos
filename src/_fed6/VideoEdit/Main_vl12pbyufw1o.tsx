...
import { FEDZ_BEATS } from "./federer_vl12pbyufw1o_beats";
import { FEDZ_BROLL } from "./federer_vl12pbyufw1o_broll";
import { TALKSZ } from "./federer_vl12pbyufw1o_hooks";
...
const TEAL = "#12B3AE";
const BG = "#0E1D23";
const AVA = "vl12pbyufw1o_opt.mp4";
...
export const TOTAL_FRAMES_VL = Math.round(VIDEO_END * 30);
...
FEDZ_BEATS.filter((b: any) => /^(hook|presenta|revela|enemigo|ciencia|colchon|fibroblastos|benef1|benef2|benef3|benef4|mito_botella|honesto|receta_intro|paso1|error|paso4|recap|close)$/.test(b.key) && (b.id.endsWith("_0")))
...
function buildWindows(): AvatarWindow[] {
  ...
  for (const b of content) {
    const forceHidden = /libro|cocina|guia|hoja_alerta|pareja|comparte|federer/.test(b.src || "");
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      <Sequence from={sec(hookStart)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)} setup="La planta olvidada en tu cocina que te borra los años…" impact="EL SECRETO DE LA SÁBILA" accentColor="#12B3AE" font={F_INTER} fontSize={130} />
      </Sequence>
