// FocusCards_vezegakr5r53.tsx — componente PROPIO del slug (aislado, no toca archivos compartidos).
// Tarjetas flotantes numeradas con imagen borrosa; a medida que el avatar dice "one… two… three…"
// (anclado al ms del caption vía item.at, en FRAMES relativos al inicio del beat) se ENFOCA la que toca.
...
export const FocusCardsVez: React.FC<{ durationInFrames: number; items: Item[]; title?: string }> = ({ durationInFrames, items, title }) => {
    ...
    <AbsoluteFill style={{ backgroundColor: BG, fontFamily: F }}>
      ...
      {title ? (
        <div style={{ position: "absolute", top: 96, width: "100%", textAlign: "center", opacity: titleOp }}>
          <div style={{ color: "#8FB6B4", fontSize: 26, letterSpacing: 6, textTransform: "uppercase", fontWeight: 700 }}>Recap</div>
