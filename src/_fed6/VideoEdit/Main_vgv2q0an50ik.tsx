...
import { FEDZ_BEATS } from "./federer_vgv2q0an50ik_beats";
import { FEDZ_BROLL } from "./federer_vgv2q0an50ik_broll";
import { TALKSZ } from "./federer_vgv2q0an50ik_hooks";
import { renderFederer2Comp, COMP2_KINDS } from "./FedererComponents2";

// ── CANAL "Federer Consejos Salud" · SUPERALIMENTO QUE REVIERTE LA PÉRDIDA MUSCULAR ─
// Avatar en 3 modos, CERO recuadro (feedback): FULL · HIDDEN (visual full) · SPLIT halfR.
const TEAL = "#12B3AE";
const BG = "#0E1D23";
const AVA = "vgv2q0an50ik_opt.mp4";
...
export const TOTAL_FRAMES_VGZ = Math.round(VIDEO_END * 30);
...
FEDZ_BEATS.filter((b: any) => /^(superalimento|leucina|resistencia|beneficio1|beneficio2|beneficio3|como_intro|paso1|paso3|error_intro|recap|resultado|close)$/.test(b.key) && (b.id.endsWith("_0")))
...
function buildWindows(): AvatarWindow[] {
  ...
  for (const b of content) {
    const forceHidden = /federer|pareja|nieta|nieto|guia|desayuno/.test(b.src || "");
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      <Sequence from={sec(hookStart)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)} setup="Un solo alimento revierte la pérdida de músculo después de los 60…" impact="Y ES EL HUEVO ENTERO" accentColor="#12B3AE" font={F_INTER} fontSize={120} />
      </Sequence>
