...
import { FocusCardsVez } from "./FocusCards_vezegakr5r53";
import { LoopLockVez } from "./LoopLock_vezegakr5r53";
import { F_INTER } from "./kit/premium/theme";
import { FEDZ_BEATS } from "./federer_vezegakr5r53_beats";
import { FEDZ_BROLL } from "./federer_vezegakr5r53_broll";
import { TALKSZ } from "./federer_vezegakr5r53_hooks";
import { renderFederer2Comp, COMP2_KINDS } from "./FedererComponents2";

// ── CANAL "Dr. Federer | Holistic Health" · ROMERO DE NOCHE = MANCHAS/MELASMA/ARRUGAS ─────────
// Avatar: FULL · HIDDEN (visual full). ⛔ CERO recuadro/PiP/split (feedback creador). Look CLÍNICO teal.
const TEAL = "#12B3AE";
const INK = "#12222B";
const BG = "#0E1D23";
const AVA = "vezegakr5r53_opt.mp4";
...
export const TOTAL_FRAMES_VEZ = Math.round(VIDEO_END * 30);
...
function buildWindows(): AvatarWindow[] {
  ...
  for (const b of content) {
    // ⛔ CERO recuadro/split en este canal (feedback creador). Ventanas de contenido = HIDDEN (visual full).
...
const renderComp = (b: any, d: number) =>
  ...
  : b.kind === "focuscards" ? <FocusCardsVez durationInFrames={d} items={b.items} title={b.title} />
  : b.kind === "looplock" ? <LoopLockVez durationInFrames={d} title={b.title} sub={b.sub} />
  : renderFederer2Comp(b, d, { medico: true });

// ── Endcard EN (inline, aislado — no toca el Endcard compartido en español) ──────────────
const EndcardVez: React.FC<{ durationInFrames: number }> = ({ durationInFrames: D }) => {
  ...
  return (
    <AbsoluteFill style={{ opacity: op, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ position: "relative", transform: `translateY(${interpolate(enter, [0, 1], [40, 0])}px) scale(${interpolate(enter, [0, 1], [0.94, 1])})`, padding: "56px 80px", borderRadius: 40, textAlign: "center", background: "linear-gradient(150deg, rgba(255,255,255,0.9), rgba(240,248,249,0.82))", backdropFilter: "blur(24px) saturate(150%)", WebkitBackdropFilter: "blur(24px) saturate(150%)", border: "1.5px solid rgba(255,255,255,0.9)", boxShadow: "0 40px 90px rgba(0,0,0,0.5)" }}>
        ...
        <div style={{ fontFamily: F_INTER, fontWeight: 900, fontSize: 88, color: INK, marginTop: 12, letterSpacing: -1, opacity: interpolate(frame, [10, 24], [0, 1], cl) }}>Subscribe</div>
        <div style={{ fontFamily: F_INTER, fontWeight: 500, fontSize: 34, color: "rgba(20,34,43,0.7)", marginTop: 14, maxWidth: 780, opacity: interpolate(frame, [16, 30], [0, 1], cl) }}>
          Every week, one natural fix the labs would rather keep quiet
        </div>
        <div style={{ display: "inline-flex", alignItems: "center", gap: 16, marginTop: 34, padding: "20px 44px", borderRadius: 999, background: `linear-gradient(150deg, ${TEAL}, #0c8f8b)`, boxShadow: `0 16px 40px ${TEAL}66`, transform: `scale(${pulse})`, opacity: interpolate(frame, [22, 36], [0, 1], cl) }}>
          ...
          <span style={{ fontFamily: F_INTER, fontWeight: 800, fontSize: 38, color: "#fff" }}>SUBSCRIBE</span>
        </div>
      </div>
    </AbsoluteFill>
  );
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      <Sequence from={sec(hookStart)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)} setup="Those spots you cover with makeup every morning…" impact="NOT A SKIN PROBLEM. A REPAIR PROBLEM." accentColor="#12B3AE" font={F_INTER} fontSize={82} />
      </Sequence>

      {/* ENDCARD */}
      <Sequence from={sec(CTA_AT)} durationInFrames={sec(Math.max(2, VIDEO_END - CTA_AT))} layout="none">
        <EndcardVez durationInFrames={sec(Math.max(2, VIDEO_END - CTA_AT))} />
      </Sequence>
    </AbsoluteFill>
  );
};
