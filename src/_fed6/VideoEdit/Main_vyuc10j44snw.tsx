...
import { FEDZ_BEATS } from "./federer_vyuc10j44snw_beats";
import { FEDZ_BROLL } from "./federer_vyuc10j44snw_broll";
import { TALKSZ } from "./federer_vyuc10j44snw_hooks";
import { renderFederer2Comp, COMP2_KINDS } from "./FedererComponents2";

// ── CANAL "Federer Consejos Salud" · COME ESTO ANTES DE DORMIR · PIERNAS FUERTES A LOS 80 ──
// Avatar en 3 modos, CERO recuadro (feedback): FULL · HIDDEN (visual full) · SPLIT halfR.
const TEAL = "#12B3AE";
const BG = "#0E1D23";
const AVA = "vyuc10j44snw_opt.mp4";
...
export const TOTAL_FRAMES_VYUC = Math.round(VIDEO_END * 30);
...
// FULL breve del avatar: arranque de cada sección/beat clave + cierre
const FULL_AT: number[] = [];
FEDZ_BEATS.filter((b: any) => /^(hook|promesa|eugster|tufts|cerdena|sarcopenia|taller|enemigo|caseina|cena_rep|ben_intro|ben3|receta|limites|error|recap|comparte|cierre|close)$/.test(b.key) && (b.id.endsWith("_0")))
...
function buildWindows(): AvatarWindow[] {
  ...
  for (const b of content) {
    const forceHidden = /libro|cocina|federer|guia|pareja|abuelo|suplementos|corredor|pastor|asilo/.test(b.src || "");
...
export const MainVYUC: React.FC = () => {
  const hookStart = 2.2;
  const hookDur = 5.2;
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      <Sequence from={sec(hookStart)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)} setup="A los 80, ya no se paraba solo del sillón…" impact="LO QUE COMIÓ ANTES DE DORMIR" accentColor="#12B3AE" font={F_INTER} fontSize={112} />
      </Sequence>
